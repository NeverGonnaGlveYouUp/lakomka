## О проекте

### Ситуация
В компании, занимающейся производством и оптовой продажей кондитерских изделий, использовалась устаревшая и неудобная система для сбора заказов PalmOrder. Эта система позволяет собирать заказы в формате XML и передавать их на сервер бухгалтерии, однако она накладывает ограничения: контрагент может создавать заказы только через агента, у которого установлено приложение PalmOrder.

### Задача
Необходимо разработать новый веб-сайт, который заменит PalmOrder, упростит процесс заказа для контрагентов и оптимизирует взаимодействие с бухгалтерией. Цель проекта - создать более удобную и современную систему для сбора и обработки заказов без зависимости от назначения агентов.

### Действия
#### Анализ требований: Провел исследование текущей системы и собрал отзывы пользователей о проблемах, с которыми они сталкиваются.
#### Проектирование: Разработал архитектуру нового веб-сайта, включая интерфейс с использованием JavaScript, React и MUI для фронтенда.
#### Реализация бэкенда: Написал бэкенд на Java 17 с использованием Spring Boot, обеспечивая взаимодействие с бухгалтерией через объектное хранилище файлов S3, обработку заказов и хранение данных.

### Результат
В результате была создана система для сбора заказов, которая значительно упростила процесс для контрагентов. Пользователи получили возможность самостоятельно создавать заказы без необходимости взаимодействия с агентами, улучшилась скорость и точность обработки заказов в бухгалтерии. Система теперь более эффективна и удобна, что положительно сказалось на удовлетворенности клиентов и работе компании в целом.

## Локальный запуск

### Запуск Хранилища S3 и Postgersql

1. Если первый запуск, выполнить создание каталогов для данных S3 И DB в корне проекта

    `mkdir -p docker/db/data`

    `mkdir -p docker/s3/data`

    `mkdir -p docker/s3/config`

    Каталоги data уже добавлены в gitignore

2. Чтобы запустить:

    Оба сервиса `docker-compose up -d` 

    S3 сервис `docker-compose up minio -d` 

    postgres сервис `docker-compose up postgres -d`

3. Чтобы остановить все сервисы `docker-compose down`

4. Bucket указанный в переменной S3_BUCKET, будет создан автоматически при запуске приложения, если еще не существовал. 

5. Web интерфейс S3: http://127.0.0.1:9001/

### Запуск бэкэнда в IDE (idea, openide)

1. В конфигурации запуска LakomkaShopApplication указать файл переменных окружения `.run/dev.env` и нужный спринг-профиль.

2. Если спринг-профиль не указан - режим продакшена, на БД выполняются новые миграции, тестданные не создаются. 
   Запросы sql не отображаются в логе.

3. Запуск приложения со спринг-профилем dev:
   - очищается БД
   - выполняется общая миграция БД через liquibase
   - выполняется миграция БД контекста dev (файл db/changelog/testdata.xml)
   - выполняется бин генерации тест-данных DatabaseInitializer
   
4. Запуск приложения со спринг-профилем jpa-dev (работа с БД без liquibase, для создания таблиц по entity, 
   для последующего создания changeset через liquibase diff):
   - очищается БД (при этом выживут таблицы liquibase: databasechangelog и databasechangeloglock)
   - выполняется миграция БД через jpa
   - выполняется миграция БД контекста dev (файл db/changelog/testdata.xml)
   - выполняется бин генерации тест-данных DatabaseInitializer
   - при останове приложения БД очищается, поэтому liquibase diff делать во время запущенного приложения

### Запуск фронта

1. Установка зависимостей:
   - установить node.js 22
   - в resources\static модуля api выполнить `npm install`, должна быть создана папка node_modules и package-lock.json
2. Запуск:
   - запуск в dev режиме: выполнить в resources\static `npm run watch`, должен быть создан bundle.js

### Сборка и запуск бека+фронта из fat-jar

1. Собрать jar `mvn clean package`, для прода `mvn clean package -Pprod` (не копируются dev yamlы)
2. Открыть терминал bash в корне проекта
3. Установить переменные окружения в ENV `source ./load_dev_env.sh`
4. Удостовериться что миграции актуальны. Если нет, то сделать дифференциальный changeset.
5. Если приложение запускалось через спринг-профиль 'jpa-dev', то БД пустая, нужно очистить таблицу databasechangelog. `delete from databasechangelog; commit;`
6. Запуск `java -jar api/target/lakomka.jar`

### Логирование

Работает логирование в память. Сохраняется 10000 строк. При достижении максимума происходит выгрузка на хранилище S3 
в виде файла logs/YYYY-MM-DD-HH-mm-ss.log.gz, после чего буфер очищается. При попытке выгрузки, независимо от
доступности S3 буфер очищается и заполняется заново.
Есть возможность скачать логи браузером из текущего буфера по урл: /api/system/logs, 
скачивается файл вида lakomka_YYYY-MM-DD-HH-mm-ss.log.zip